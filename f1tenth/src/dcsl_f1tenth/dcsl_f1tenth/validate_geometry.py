...
from math import acos, sin, cos, nan
import math
...
class OdometryValidation(Node):
    ...
    def odom_callback(self, msg):
        ...
        while (
            len(self.odom_hist) > 0
            and to_sec(self.odom_hist[0].header.stamp) < ts - self.duration - 1
        ):
    ...
    def vicon_callback(self, msg):
        ...
        while (
            len(self.vicon_hist) > 0
            and to_sec(self.vicon_hist[0].header.stamp) < ts - self.duration - 1
        ):
    ...
    def get_interpolated_odom_at_t(self, t, history):
        ...
        msg.pose.pose.position = self.interpolate_pos(
            before.pose.pose.position,
            after.pose.pose.position,
            to_sec(before.header.stamp),
            to_sec(after.header.stamp),
            t,
        )
        msg.pose.pose.orientation = self.interpolate_quat(
            before.pose.pose.orientation,
            after.pose.pose.orientation,
            to_sec(before.header.stamp),
            to_sec(after.header.stamp),
            t,
        )
        msg.twist.twist.linear = self.interpolate_vec3(
            before.twist.twist.linear,
            after.twist.twist.linear,
            to_sec(before.header.stamp),
            to_sec(after.header.stamp),
            t,
        )
        msg.twist.twist.angular = self.interpolate_vec3(
            before.twist.twist.angular,
            after.twist.twist.angular,
            to_sec(before.header.stamp),
            to_sec(after.header.stamp),
            t,
        )
    ...
    def is_finite_odom(self, msg: Odometry) -> bool:
        """Return False if any position or twist element is NaN or Inf."""
        vals = [
    ...
    def update_prediction(self):
        ...
        if not self.vicon_hist_ready or not self.odom_hist_ready:
            ...
            return
        tt = np.linspace(t.nanoseconds * 1e-9 - self.duration-0.1, t.nanoseconds * 1e-9-0.1)
        # DEBUG FIXME find out why we are extrapolating
        # to_sec(self.odom_hist.header.stamp)
        odom_raw   = [self.get_interpolated_odom_at_t(t, self.odom_hist)   for t in tt]
        vicon_raw  = [self.get_interpolated_odom_at_t(t, self.vicon_hist)  for t in tt]

        # 2. filter out any invalid entries
        odom_vec, vicon_vec = [], []
        for i, (o, v) in enumerate(zip(odom_raw, vicon_raw)):
            if not self.is_finite_odom(o):
                self.get_logger().warning(f"odom[{i}] has NaN/Inf → skipping")
                continue
            if not self.is_finite_odom(v):
                self.get_logger().warning(f"vicon[{i}] has NaN/Inf → skipping")
                continue
            odom_vec.append(o)
            vicon_vec.append(v)

        if not odom_vec or not vicon_vec:
            self.get_logger().error("No valid odometry/vicon data after NaN/Inf filtering")
            return

        # 3. logging (only valid samples)
        '''
        for i, (odom_msg, vicon_msg) in enumerate(zip(odom_vec, vicon_vec)):
            px, py = odom_msg.pose.pose.position.x, odom_msg.pose.pose.position.y
            vx, vy = vicon_msg.pose.pose.position.x, vicon_msg.pose.pose.position.y
            self.get_logger().info(
                f"[{i}] t={tt[i]:.2f} → odom=({px:.3f},{py:.3f}), vicon=({vx:.3f},{vy:.3f})"
            )
        '''
